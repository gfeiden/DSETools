...
class MassTrack(object):
    ...
    def __init__(self, mass, metallicity, alpha_enhancement = 0.0):
        ...
        track_directory = dlib.trk_directory
        ...
        self.filename   = 'm{:04.0f}_GS98_{:s}{:.0f}_{:s}{:01.0f}_T60.iso'.format(
                           self.mass*1000., feh_letter, abs(self.feh),
                           afe_letter, abs(self.afe))
    ...
    def inRange(self):
        ...
        if not min(dlib.feh_list) <= self.feh <= max(dlib.feh_list):
            print '\nRequested [Fe/H] = {:+6.2f} is out of range.\n'.format(self.feh)
            in_range = False
        else:
            in_range = True
            
        if not min(dlib.afe_list) <= self.afe <= max(dlib.afe_list):
    ...
    def inLibrary(self):
        ...
        """
        if self.mass not in dlib.mass_list:
            self.mass_exists = False
        else:
            self.mass_exists = True
        
        if self.feh not in dlib.feh_list:
            self.feh_exists = False
        else:
            self.feh_exists = True
        
        if self.afe not in dlib.afe_list:
